...
dev:
	...
	start /B cmd /c "cd tic-tac-toe && npm run dev"
	start /B cmd /c "cd pixel-battle && npm run dev"
	start /B cmd /c "cd gartic-phone && npm run dev"
...
logs:
	...
	dir tic-tac-toe\*.log 2>nul || echo "No logs found"
	echo "Pixel Battle logs:"
	dir pixel-battle\*.log 2>nul || echo "No logs found"
	echo "Gartic Phone logs:"
	dir gartic-phone\*.log 2>nul || echo "No logs found"
...
health:
	...
	curl -s http://localhost:3000/health >nul 2>&1 && echo "✅ Tic-Tac-Toe: Healthy" || echo "❌ Tic-Tac-Toe: Down"
	curl -s http://localhost:3001/health >nul 2>&1 && echo "✅ Pixel Battle: Healthy" || echo "❌ Pixel Battle: Down"
	curl -s http://localhost:3002/health >nul 2>&1 && echo "✅ Gartic Phone: Healthy" || echo "❌ Gartic Phone: Down"